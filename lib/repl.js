...
(function() {
  ...
  autocomplete = function(line, cb) {
    ...
    try {
      var prediction = compiler.predict(line, line.length);
      var prefix = '';
      if (matches = line.match(/^(.+)(its|\.|->)\s*([a-zA-Z0-9_\s]*)$/)) {
        prefix = matches[3];
      } else if (matches = line.match(/^([a-zA-Z0-9_\s]*)$/)) {
        prefix = matches[1];
      }
      if (prediction) {
        ...
        if (typeHintNode) {
          ...
          if (allProps) {
            var matchingProps = allProps.map(function (p) {
              return p.singularPhrase;
            }).filter(function(p) {
              if (p === undefined) {
                return false;
              }
