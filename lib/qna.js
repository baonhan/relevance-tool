...
(function() {
  ...
  qnaCommand = qnaPath + " -showtypes " + qnaInputFile + " " + qnaOutputFile;
  var qnaOutputFile = "/tmp/output";

  if (process.platform == "win32") {
    qnaPath = path.resolve(__dirname, '../qna/QNA9.1.1245.0/eval.exe');
    qnaInputFile = path.resolve(__dirname, '../qna/QNA9.1.1245.0/input.qna');
    qnaOutputFile = path.resolve(__dirname, '../qna/QNA9.1.1245.0/output');
    qnaCommand = qnaPath + " " + qnaInputFile + " " + qnaOutputFile + " Types";
  }
    ...
    return fs.writeFile(qnaInputFile, qnaContent, function(err) {
      if (err) {
        ...
        return childProcess.exec(qnaCommand, function(error, stdout, stderr) {
          ...
          fs.readFile(qnaOutputFile, 'utf8', function (err,data) {
            if (err) {
